...
function Login() {
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    try {
      // requests.get(endpoint, auth=("login", "password")).json()
    // supongo que debo mandar primero una log a auth, traigo el token y luego 
    //lo uso en los headers para crear el user en db oooo... directo log en cada funcion view
      await axios({
        method: "GET",
        // headers: {'Authorization': {username:log.}},
        // auth:'username:user, password:django',
        url: `http://localhost:8000/user/login/${log.username}`
      }).then((res) => {
        localStorage.setItem("NicaMovieUser", res.data.id_user);
        setCon(!con);
        setLog({ username: "", password: "" });
      });
    } catch (error) {
      return alert(`no user "${log.username}" founded or pass incorrect, try again please`);
    }
  }
